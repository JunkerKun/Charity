...
Object* CollisionCheckPoint(float x, float y, unsigned int index) {
	...
	int chunkXStart = std::max(0,chunkX);
	int chunkYStart = std::max(0,chunkY);
	int chunkXEnd = std::min(engine->objectsManager->chunksNumber.x,chunkXStart+1);
	int chunkYEnd = std::min(engine->objectsManager->chunksNumber.y,chunkYStart+1);
