...
sf::Vector2i Input::GetMousePosition(bool absolute) {
	if (absolute) {
		sf::Vector2i vec;
		vec.x=engine->camera->xView+mousePosition.x;
		vec.y=engine->camera->yView+mousePosition.y;
		return vec;
	};
...
sf::Vector2i Input::GetLastMousePosition(bool absolute) {
	if (absolute) {
		sf::Vector2i vec;
		vec.x=engine->camera->xView+mousePositionLast.x;
		vec.y=engine->camera->yView+mousePositionLast.y;
		return vec;
	};
